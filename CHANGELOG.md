...
- Dictionaries are plain string-replacement lists (`string,replacement,type`) applied to the text processed in the run: to the notes before `--translate`, or to the narration text before `--tts`. Translation and synthesis are run separately when they need different dictionaries; `--dict-file` is repeatable.
